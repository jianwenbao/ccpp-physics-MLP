...
!!
      subroutine GFS_PBL_generic_post_run (im, levs, nvdiff, ntrac,                                                            &
        ...
        ugrs, vgrs, tgrs, qgrs, save_u, save_v, save_t, save_q, huge,           &
        do_mlp_pbl, tpbltend, qpbltend, upbltend, vpbltend,errmsg, errflg)
...
!mlp
      real(kind=kind_phys), dimension(im,levs),intent(inout) :: tpbltend,qpbltend,upbltend,vpbltend
...
!jwb/sam mlp
      if(do_mlp_pbl) then
             !print*,"getting tend pbl post"
                tpbltend(1:im,1:levs)=dtdt(1:im,1:levs)*dtf!tgrs(1:im,1:levs) - save_t(1:im,1:levs)
                upbltend(1:im,1:levs)=dudt(1:im,1:levs)*dtf!ugrs(1:im,1:levs) - save_u(1:im,1:levs)
                vpbltend(1:im,1:levs)=dvdt(1:im,1:levs)*dtf!vgrs(1:im,1:levs) - save_v(1:im,1:levs)
                qpbltend(1:im,1:levs)=dqdt(1:im,1:levs,ntqv)*dtf!qgrs(1:im,1:levs,ntqv)-save_q(1:im,1:levs,ntqv)
      endif   ! end imlp_pbl
